...
扣除商品、返还牛奶空桶和金币入账视为一个事务：操作前复制玩家主物品栏与全部授权箱槽，任何扣货不足、返还空间不足或金币写入失败都会逐槽恢复并标脏。交易快照的现有 `remainingItems` 字段在出售项上解释为授权箱库存，在购买项上仍是商人库存；客户端把前者与实时本地主背包计数相加，仅用于显示和按钮体验，最终数量继续由服务端重新验证。用户未指定配方，因此不推断材料或增加未经确认的获取玩法。
