...
决定：商人购买的“一组”不再由目录填写数量，而是始终由目标 `Item` 的 Java 1.12.2 `getItemStackLimit()` 派生。价格和有限库存以组计，GUI 显示每组实际物品数及剩余实际物品数。`DailyOffer` 的持久字段名仍为 `remainingBundles`，但其数值含义固定为剩余组数。行情书历史目录扩展为全部 22 种可出售给商人的逻辑商品；16 色羊毛继续共用一条逻辑商品和一条曲线。蟹笼的 20 格全部是通用容器槽，`ISidedInventory` 对所有方向公开全部槽位；槽 0/1 仅在运行时分别读取钓竿/合法生肉，捕捞结果仍只尝试放入 2～19。每轮基础等待改为 2,000～12,000 tick（100～600 秒），Lure 每级减少 2,000 tick（100 秒）。羊毛转线配方用 1.12.2 wildcard metadata 接受全部 16 色羊毛。
...
兼容性与影响：不更改 registry ID、Packet discriminator、`PastoralWorldData` 根版本或 NBT key。旧市场数据首次访问时本来就重建当前世界日的完整 30 日窗口，因此新增的 8 条商品曲线会被确定性补齐。已保存的当天商人 Offer 保留其 `remainingBundles` 数值，但从加载后开始按新的原版整组输出；跨日会按新规则重新生成。蟹笼的既有 20 槽物品和倒计时 NBT 均可读取，但已开始的一轮将在当前规则下继续处理并在下次重新抽取时使用新的等待范围。
...
决定：默认链式市场对基础价大于 1 金币的商品设置 2 金币低价保护，正常日推进不会把价格降到 1。若链式输入已经是 1 金币（例如旧存档快照），下一次推进无论抽到的方向如何都至少返回 2；正向波动在整数取整后没有增长时也至少增加 1 金币。`moreStableMarketVolatility=true` 继续使用原有独立三角分布和 1 金币下限，确保兼容模式不被悄然改写。

原因：百分比步长作用于 1～2 金币时，`Math.round` 会把上涨结果舍回原价；同时下跌再经 1 金币下限会形成永久平台。低价保护和方向保持规则解决冻结点，不建立新的百分比上限，也不重算已持久化当天价格。

兼容性与影响：只修改 `MarketPriceGenerator` 的默认链式单日计算，不变更 market NBT 结构、WorldSavedData 版本、Packet、registry ID 或配置键。旧的 1 金币当天快照保持原值，直到自然跨入下一日才按新规则恢复；同日重登、打开界面和切换设置均不刷新价格。
