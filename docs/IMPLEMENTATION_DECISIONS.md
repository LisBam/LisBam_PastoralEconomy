...
- 成年牛的挤奶冷却归属于牛实体，而非玩家；最近一次成功挤奶 tick 写入 `Entity#getEntityData()`，因此区块卸载、重启和多人共享同一头牛时仍保持 6000 tick 冷却。启用冷却时客户端取消本地 `EntityCow` 预测，但不会阻止此前已经发出的交互包；这样真实牛奶桶只由逻辑服务端的原版成功分支创建。`disableMilkingCooldown=false` 为默认安全值，开启后两侧均不拦截原版交互。非冷却交互绝不手动扣空桶或制造牛奶桶，而是记录时间后交还原版路径；冷却中的交互才在逻辑服务端拒绝。
