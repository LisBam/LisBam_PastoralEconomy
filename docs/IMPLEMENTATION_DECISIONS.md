...
决定：`drop_attraction` 使用新 registry ID，不复用或改名既有附魔；它是稀有、I 级、非宝藏附魔，使用独立的斧/镐/锹/锄/剪刀白名单，不能复用包含剑的 Range 白名单。方块掉落在 `HarvestDropsEvent` 的最终堆栈表形成后才替换为受控 `EntityItem`，并保留事件给出的每栈掉落概率。库存不会由事件处理器直接插入；实体抵达后仍由 1.12.2 原版拾取逻辑结算，满背包时只冻结实体的飞行状态。剪羊毛没有通用的方块收获事件，因此在成功交互期间以目标羊/哞菇和同世界近距离实体加入事件关联原版掉落；丰收补发栈在生成时直接关联。所有关联均为服务端瞬态状态，重启时未抵达的物品恢复普通实体即可，无需 NBT 迁移。

原因与影响：使用原版拾取可保持堆叠、事件、背包限制与多人同步的唯一服务端权威路径，避免复制或吞物。新附魔 ID 追加注册，不影响既有物品、世界数据、包编号或存档迁移；萤石粉仅更改目录基础价，已有行情和 Offer 保持既有“不重写”策略。
...
原版 Mending 只随机扫描手持与盔甲而不会看到 Capability 肩部栈，故仅在损坏的经验修补羽毛翅膀捡起经验球时取消该次原版处理，按原版修补比例修复并把余下经验给予玩家。绑定诅咒复用 Slot 的原版取下检查；死亡 Clone 则先复制能力再清空带消失诅咒的肩部栈。客户端通过仅客户端 `RenderPlayer` layer 渲染 `ModelElytra`，追踪者已通过 Packet 9 收到翅膀栈，不向其泄露背包 NBT。

原因与影响：肩部栈既是已保存、已同步且已阻止胸甲鞘翅绕过的唯一装备所有权边界。使用原版能力同步而不是客户端篡改可令 Dedicated Server 验证是否仍穿戴；临时内存 flight counter 跨登出无需保留。用户没有指定交易价格，采用同一珍宝池内原版鞘翅的 1200000 基准，避免另建价格系统。新增 market/catalog key 是追加式，旧存档的现有 Offer 与所有 schema/协议完全兼容。

## DEC-060 输入框背包键、丰收斧头与凭证箱票据（2026-09-06）

决定：所有自定义 GUI 继续用 Esc 和打开背包改键关闭其服务端 Container，但仅在没有 `GuiTextField` 焦点时处理后者；文本框聚焦后交由原版输入框处理。Harvest 的稳定 ID、最大 III、时运互斥和书本可得性不变，五把原版斧头加入其可应用范围，但作为逐物品附魔台排除项，斧头只能经附魔书/铁砧获得；农业服务端流水线同时把锄头或斧头视为作物 Harvest 工具。田园眷顾随后维护为 I--IV `20/40/60/80%`，只在原有单次种植/收获判定中直接奖励 1 点经验。
...
决定：生存 `ContainerPlayer` 的真实肩部 Slot 固定在原版副手 `77,62` 正上方 `77,44`；创造模式只在“生存物品栏”页展示该槽，并按其重排后的副手 `35,20` 放到 `35,2`。两种模式都直接裁取原版 `inventory.png` 的同一盔甲槽画面，故资源包替换该原版纹理时肩部槽同步变化。手持鞘翅右键使用双端 `RightClickItem` 拦截，客户端只返回结果，逻辑服务端检查并写入 Capability。`LayerElytra` 通过已有 Coremod 的同一 Object 描述符 hook 改读肩部；远端玩家展示由追加的 S2C Packet 9 同步，而不是让客户端猜测或把肩部伪装成胸甲装备。
...
## DEC-057 交易凭证授权与多库存出售事务（2026-09-06）

决定：只新增一个稳定 Item ID `lisbam_pastoral_economy:trade_voucher` 的空白/绑定双状态物品。空白凭证右键只由逻辑服务端写入使用者 UUID 和绑定时名字，已经存在合法 UUID 后不可重绑。名字只用于本地化显示，箱子授权始终比较 UUID。商人服务在现有服务器主线程交易入口扫描服务器所有已加载维度的已加载原版 `TileEntityChest`，不强制加载区块；大箱去重为一个库存，上锁箱与未展开战利品箱不参与。玩家主物品栏排在箱子槽之前，授权箱按维度/坐标排序。

扣除商品、返还牛奶空桶和金币入账视为一个事务：操作前复制玩家主物品栏与全部授权箱槽，任何扣货不足、返还空间不足或金币写入失败都会逐槽恢复并标脏。交易快照的现有 `remainingItems` 字段在出售项上解释为授权箱库存，在购买项上仍是商人库存；客户端把前者与实时本地主背包计数相加，仅用于显示和按钮体验，最终数量继续由服务端重新验证。空白凭证按明确需求采用八个金粒环绕中央纸张的有序配方。

原因：把箱子内容复制进玩家背包或新增箱子 TileEntity 所有权会制造额外同步、掉落与旧存档边界；凭证本身已能作为可移动授权令牌。只处理当前加载的原版箱子既避免无界区块加载，也能用原版双箱和锁语义；一次快照覆盖所有来源，才能避免跨库存扣了一半后因空桶或金币失败产生丢失/复制。UUID 不受重名与改名影响，绑定时名字仍能满足物品可读性。
...
## DEC-059 维护：死亡金币、基础价回归与已删除物品兼容（2026-09-06）

决定：真实玩家的 `LivingDeathEvent` 在逻辑服务端末端通过既有 `CoinService` 扣除随机 10%--30% 的金币；若此值低于本次随机的 1000--2000，则采用后者，最终最多扣到当前余额，并直接向该玩家发送红色中文聊天组件。扣款在玩家 Clone 之前完成，不新增 Capability、Packet 或 NBT 字段。动物额外骨头概率的三个成功档统一改为原来的 2/3。肩部栏无论生存或创造“生存物品栏”页，均裁取同一张原版 `inventory.png` 的盔甲槽像素，继续跟随资源包。

手动基础价调整绝不重写 `PastoralWorldData` 中的当前/昨日快照或历史；后续每个世界日从保存价格按既有步长/回归概率前进。旧值在新价格带外时，持续按日回归，重新进入后才恢复硬性价格带钳制；默认链式模式和可选独立稳定模式都遵循这条迁移规则。空交易凭证使用纸置中、八个金粒环绕的有序 JSON 配方。旧交通定位物品的注册、行为和资源全部删除，但 Item 缺失映射明确 `ignore`，使已存在的旧物品栈在载入时安全消失。

原因：死亡金币、交易、价格和持久化状态必须只在逻辑服务端通过已有权威服务修改。直接把保存行情钳到新基础价价格带会让调价日产生突变，也会违反“旧存档价格不动”的兼容承诺；稳定模式若直接采用独立新样本也有相同问题。使用原版同一张槽纹理而非创造页专用纹理，能让两个模式视觉统一并自然支持资源包。删除旧物品时若不处理 Forge 1.12.2 `MissingMappings<Item>`，旧世界可能因未知 registry 名称无法安全载入。

兼容性与影响：不增加或重排 Packet discriminator，不修改玩家/世界/方块 NBT schema、WorldSavedData 名称或任何仍存在的 registry ID。旧世界的已删除物品会消失；现有市场价格在调价当天保持字节值不变，之后自然回归。新空凭证配方只影响后续合成可得性。
