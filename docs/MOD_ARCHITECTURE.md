...
- 资源：`mcmod.info`、`pack.mcmeta`（format 3）、中英文 `.lang`、正式 Logo、行情书资源和蟹笼的 blockstate、Block/Item model、合成配方均已存在；`textures/blocks/crab_trap.png` 与 `transport_station.png` 都是 32×32、最近邻像素风 PNG。`village_station` 与玩家可放置的交通站共用后者，不再引用原版橡木木板；两种交通站均有正确的 `.name` 本地化。行情书模型明确引用原版 `minecraft:items/book_normal`，不另行复制位图。
