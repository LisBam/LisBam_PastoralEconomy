...
- 行情书请求在 40 client tick 未收到快照时会重试，服务端既有的 2 tick 合并节流不再能造成永久“正在读取行情”。背景整块裁取原版 `demo_background.png`，文本仍使用 `FontRenderer`。
