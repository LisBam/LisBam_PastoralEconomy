...
- 世界数据：`data.world.PastoralWorldData`（`lisbam_pastoral_economy_world_data`）是唯一 WorldSavedData 根，`dataVersion=7`。访问器始终解析服务器主世界 MapStorage，保证跨维度只有一份共享数据。除首次初始化标记和 `farmHarassment` 短期记录外，v7 `market` 段保存持久 marketSeed、首次市场日、当前/昨日快照和 14 种作物的最近 30 天；不再持久化无限 `processedDays` 或无限历史。v3--v6 读取后在首次市场访问时按稳定 seed 重建当前窗口，并在下一次保存写成 v7；merchant 段保存村庄、站点、商人、Offer、库存、轮换状态和附魔书已解析等级；transport 段保存 SELF_BUILT/VILLAGE 物理节点 UUID、维度、坐标和村庄身份引用。
...
- 客户端：`client.ClientPlayerState` 与 `ClientMarketState` 都是非持久、非权威缓存，连接/断开时清空。市场缓存还有明确书本会话：打开时清空并开始接收、关闭时清空并拒绝迟到包，连接内请求号保持递增；`GuiMarketBook` 只在打开/切换作物时请求并缓存图表节点，关闭后不保留市场工作。`ContainerMarketBook` 对同一打开会话仍最多每 2 tick 生成一次历史快照；快速切换期间不再丢弃请求，而是合并为最新一项并在冷却结束后的 Container tick 发送，因此客户端不会卡在读取状态。`GuiMerchantTrade` 的数量滑条、输入框与页签偏好均为本地展示状态，出售持有量按客户端 tick 缓存，仍只发送既有有界数量请求。`ClientModelRegistry` 是物理客户端模型注册。`ClientHudEventHandler` 仍在 `RenderGameOverlayEvent.Text` 中右上角渲染金币。
...
- 村庄与商人：`merchant.VillageService` 仅在主世界低频观察 1.12.2 `VillageCollection`，按 128/64/160 格参考值去重并维护稳定 `VillageRecord`；每轮维护只扫描一次已加载实体来清除异常/重复商人并建立活动索引。`StationRecord` 与 `TileVillageStation` 保存受保护村庄站点身份，站点异常消失时在已加载安全位置恢复。`MerchantRecord`、`DailyOfferState`、`OfferRotationState` 和共享库存均在 `PastoralWorldData` v5 的 merchant 段持久化；`EntityMerchant` 仅保存身份引用，死亡/重启由 reconciliation 按 `clamp(ceil(villagers/5),3,8)` 补足，离站返航最多每 20 tick 重算一次路径。`TradeCatalog` 集中定义 6 条收购池、普通/罕见/稀有/珍宝购买池、1.12.2 Item/meta、bundle、附魔书等级和有限库存。
