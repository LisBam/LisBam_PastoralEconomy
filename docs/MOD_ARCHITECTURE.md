...
- `core.EnchantingTableCorePlugin` 是随发行 JAR 加载的最小 1.12.2 Coremod：只补丁 `Item#getItemEnchantability(ItemStack)` 与原版效率的附魔台筛选，使剪刀按铁工具、各锄头按自身材质获得附魔台候选；剪刀效率直接为原版 `Efficiency`。剪刀仍可使用模组 Harvest/Range 以及原版耐久、经验修补、消失诅咒等合法附魔；Harvest 剪羊/剪哞菇增产沿用既有成功后结算流水线。
