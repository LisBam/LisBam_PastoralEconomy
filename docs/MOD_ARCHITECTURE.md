...
- GUI：`gui.GuiIds.MARKET_BOOK=0`、`MERCHANT_TRADE=1`、`CRAB_TRAP=2`、`TRANSPORT_STATION=3`。`ModGuiHandler` 在服务端返回无槽行情书、绑定实体的商人、坐标绑定的 `ContainerCrabTrap` 或 `ContainerTransportStation`；客户端经 Proxy 返回相应 GUI，common side 不加载 `GuiScreen`。`GuiMarketBook`、`GuiMerchantTrade` 与 `GuiTransportStation` 都从实际 `ScaledResolution` 尺寸推导面板坐标；行情书和商人图标使用原版 `GuiScreen#renderToolTip`。`ClientMerchantTradeViewState` 仅在物理客户端保存本次游戏会话最后选择的出售/购买页，绝不参与交易请求或存档。`GuiCrabTrap` 复用 `generic_54` 的标题/玩家背包片段，但只手绘 2 个专用槽和 18 个收获槽的原版式槽框，绝不渲染没有 Container Slot 的虚假格子；状态文字处于空白区域。`SyncTransportStateMessage` 的每个 `TransportNodeView` 附带服务端计算的目的地旅行费或不可用标记，客户端据此显示费用并禁用不可旅行/余额不足的操作。
