...
- GUI：`gui.GuiIds.MARKET_BOOK=0`、`MERCHANT_TRADE=1`、`CRAB_TRAP=2`、`TRANSPORT_STATION=3`。`ModGuiHandler` 在服务端返回无槽行情书、绑定实体的商人、坐标绑定的 `ContainerCrabTrap` 或 `ContainerTransportStation`；客户端经 Proxy 返回相应 GUI，common side 不加载 `GuiScreen`。所有 UI 文字使用原版 `FontRenderer`，物品悬停使用原版 `GuiScreen#renderToolTip`。`GuiMerchantTrade`、`GuiTransportStation` 每帧以一次完整裁取绘制原版 `demo_background.png`，不把该纹理拆成九宫格；按钮/输入框/滑条保持原版控件。商人和交通窗口的非按钮文字采用原版工作台/熔炉标题深灰色 `0x404040`，实际按钮保留原版按钮浅色及 `enabled` 交互状态。商人物品底框直接裁取 `generic_54.png`，有限库存显示为实际剩余物品数量（剩余组数 × 每组实际物品数）。`ClientMerchantTradeViewState` 仅在物理客户端保存本次游戏会话最后选择的出售/购买页，绝不参与交易请求或存档。`GuiCrabTrap` 继承 `GuiContainer` 的原版槽位悬停路径，复用 `generic_54` 的标题/玩家背包片段、原始背景像素和逐格裁取的 20 个实际槽位；0 仅接受钓竿、1 仅接受合法生肉、2～19 为通用仓储/捕捞产出槽。`GuiTransportStation` 自动请求最近未接入村庄，仅显示一个接入按钮；确认层只展示服务端快照的距离/接入费、按余额禁用确认，最终仍发送既有 `CONNECT_VILLAGE` 请求供服务端重算。节点移出即删除个人记录，快照会清理历史无效/未激活记录，不渲染“已移出”或“节点无效”节点；选中节点名称和费用为绿色，坐标没有维度后缀。`SyncTransportStateMessage` 的每个 `TransportNodeView` 附带服务端计算的目的地旅行费或不可用标记，客户端据此显示费用并禁用不可旅行/余额不足的操作。
