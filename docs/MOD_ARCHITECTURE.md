...
- 市场：`market.MarketCatalog` 是唯一不可变商品目录，保存稳定经济 key、1.12.2 Item/meta、基础价、8 类单日波动率和历史标志。默认 `MarketPriceGenerator` 从前一天持久化价格按类别 25%～100% 的波动率步长上涨或下跌；低于基础价时 65% 概率上涨、高于基础价时 65% 概率下跌，因此偏向回归初始价格但没有固定百分比上下界。基础价高于 1 金币的商品在链式模式下设 2 金币低价保护；若价格已为 1，下一次日推进会至少恢复到 2，且正向小波动取整不变时至少增加 1 金币，避免低价冻结。设置开启时才恢复旧的 marketSeed/worldDay/key 独立三角分布（仍保留旧的 1 金币下限）。`MarketService` 是唯一服务器市场 API：行情书打开请求或实际经济读取才惰性推进到当前日，主世界不再每 tick 推进市场；为保证链式价格，跨多日会逐日推进并只持久化最近 30 天的收购历史。`MarketPriceSnapshot` 为商人单次同步批量提供当前/昨日价格，`MarketHistorySnapshot` 最多 30 点且不再提供更早页。
