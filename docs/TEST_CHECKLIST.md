...
- [x] UI 文字使用原版 `FontRenderer` 的标准深灰色；蟹笼 20 个实际槽和商人物品槽直接裁取原版 `generic_54.png`，蟹笼空白背景也取原版纹理像素；商人/交通面板以 `demo_background.png` 九宫格固定原始 1 像素边缘，不整体拉伸且没有手绘槽框/凸起边框。PASS：源码审查、`compileJava`、`build` 与 Forge audit。
