...
## 文档：GitHub 项目页 README 与 v1.7 Release 说明（2026-09-09）

实现：新增仓库根目录 `README.md`，作为面向下载者的中文简介，内容全部取自 `聆竹の休闲田园经济_模组内容书.md`：模组定位与核心循环、金币与每日行情、市场行情书、绿宝石现货、商人 NPC、交易凭证与出货箱、蟹笼、十三种新增附魔、交通网络与肩部装备、区块加载器、休闲化生存规则、配置项、源码构建方式与文档索引。同时新增 `docs/RELEASE_NOTES_v1.7.md`，作为 GitHub Releases 页的正文模板，包含安装步骤、本版内容摘要、文件大小与 SHA-256 校验值以及兼容性说明。
